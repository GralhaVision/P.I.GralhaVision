...
def cad_alt(selec):
    ...
    for x in produtos:
        if x.get('cod') == selec:
            produtos[selec]['desc'] = input(
                "Digite a nova descrição do produto: ")
            produtos[selec]['cod'] = int(
                input("Digite o novo código do produto: "))
            produtos[selec]['qtd'] = int(
                input("Digite a nova quantidade do produto: "))
            produtos[selec]['cust'] = float(
                input("Digite o novo custo do produto: "))
            produtos[selec]['prec'] = float(
                input("Digite o novo preço do produto: "))
            cad_att(produtos)
...
while True:
    ...
    if esc == 2:
        ...
        cad_list()
        if not produtos == []:
            selec = input("Informe o código de barras do produto a ser alterado: ")
            index = None
            for p in produtos:
                if p['cod'] == selec:
                    index = produtos.index(p)
                    print(index)
                    produtos[index]['desc'] = input("Digite a nova descrição do produto: ")
                    produtos[index]['cod'] = input("Digite o novo código do produto: ")
                    produtos[index]['qtd'] = input("Digite a nova quantidade do produto: ")
                    produtos[index]['cust'] = input("Digite o novo custo do produto: ")
                    produtos[index]['prec'] = input("Digite o novo preço do produto: ")
                    cad_att(produtos)
                    print("Dados alterados. ")
